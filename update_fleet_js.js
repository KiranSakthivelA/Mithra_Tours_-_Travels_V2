...
function initFleetShowcase() {
    ...
    function updateCoverflow() {
        ...
        const spacingX = isMobile ? 180 : (isTablet ? 250 : 320);
        const rotateStep = isMobile ? 24 : 28;
        ...
        visibleCards.forEach((card, i) => {
            ...
            if (diff === 0) {
                // 1. Center Active Card
                card.classList.add('active');
                card.style.transform = \`translateX(0px) translateZ(80px) rotateY(0deg) scale(\${isMobile ? 1.0 : 1.05})\`;
                ...
                card.style.transform = \`translateX(-\${spacingX}px) translateZ(-40px) rotateY(\${rotateStep}deg) scale(0.86)\`;
                ...
                card.style.transform = \`translateX(-\${spacingX * 1.85}px) translateZ(-120px) rotateY(\${rotateStep * 1.25}deg) scale(0.72)\`;
                card.style.zIndex = '10';
                card.style.opacity = isMobile ? '0' : '1';
                card.style.visibility = isMobile ? 'hidden' : 'visible';
                card.style.filter = 'brightness(0.68)';
                ...
                card.style.transform = \`translateX(\${sign * (spacingX * 2.2)}px) translateZ(-220px) scale(0.55)\`;
                card.style.zIndex = '1';
...
fs.writeFileSync(path.resolve(__dirname, 'v2/js/fleet-showcase.js'), jsContent, 'utf8');
fs.writeFileSync(path.resolve(__dirname, 'js/fleet-showcase.js'), jsContent, 'utf8');
console.log('Updated both JS files with zero-opacity ghosting fix!');
