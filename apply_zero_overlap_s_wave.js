...
const zeroOverlapSWaveCss = `/* ═══════════════════════════════════════════════════════════
   WHY MITHRA — TRUE S-WAVE ROADMAP (CONNECTED ICONS & ZERO OVERLAP)
   ═══════════════════════════════════════════════════════════ */
...
/* Top Steps (Step 1 & Step 3): Text in Top Zone, Icon at Y ~ 180px */
.flow-node-item.pos-top {
    justify-content: flex-start;
    padding-top: 0.5rem;
}

/* Bottom Steps (Step 2 & Step 4): Icon at Y ~ 280px, Text in Bottom Zone */
...
/* Align squircle to the center of column */
    ...
    <div class="container">
        ...
        <div class="why-flow-wrapper">
            ...
            <div class="flow-steps-grid">
                <!-- 1. One Vendor, Every Need (Top Crest: Text in Top Band, Icon on Road) -->
                ...
                <!-- 2. Consistency You Can Verify (Bottom Trough: Icon on Road, Text in Bottom Band) -->
                ...
                <!-- 3. Safety First (Top Crest: Text in Top Band, Icon on Road) -->
                <div class="flow-node-item pos-top">
                    <div class="flow-node-body">
                        <h3>Safety First</h3>
                ...
                <!-- 4. On-Time, Every Time (Bottom Trough: Icon on Road, Text in Bottom Band) -->
                <div class="flow-node-item pos-bottom">
                    <div class="flow-squircle-icon">
...
['v2/css/brand.css', 'css/brand.css', 'deploy_ready/css/brand.css'].forEach(f => {
    ...
    const regex = /\/\* ═+\r?\n\s*WHY MITHRA — [\s\S]*?(?=$)/;
    if (regex.test(css)) {
        css = css.replace(regex, zeroOverlapSWaveCss.trim());
...
['v2/index.html', 'index.html', 'deploy_ready/index.html'].forEach(f => {
    ...
    if (regex.test(html)) {
        html = html.replace(regex, zeroOverlapSWaveHtml);
        fs.writeFileSync(file, html, 'utf8');
    }
});

console.log('Applied zero-overlap true S-wave roadmap!');
